...
First, if you haven't already: [download awful](https://awful.wtf/download).

**I highly recommend you get these AddOns.** They're invaluable tools that'll save you a lot of time.

- [idTip](https://www.curseforge.com/wow/addons/idtip-community-fork) - Display spellIDs in spell, talent, aura, and item tooltips.

- [BugSack](https://www.curseforge.com/wow/addons/bugsack) - Store Lua errors & their full debug stack to view later.
...
#### Enter your project's name as a variable:
...
- Enter routines, and create your project folder: awful/routines/**<Observe>{({ setState, ...state }) => (<>{state.name}</>)}</Observe>**.
...
> awful/routines/example/**<Observe>{({ setState, ...state }) => (<>{state.name}</>)}</Observe>.lua**
...
- We're calling this file our **spell book** because it will contain a list of our spells as **spell objects**, created with `awful.Spell`
